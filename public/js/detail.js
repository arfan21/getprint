...
app.controller("appCtrl", ['$scope','$http','$window', function($scope,$http,$window){
    ...
    $scope.updateRating= () => {
        ...
        if(uidlineSama(uidline,user_rating)){
            $scope.data.rating.total_point = $scope.data.rating.total_point - user_rating[i].rating_user;
            user_rating[i].rating_user = point;
            $scope.data.rating.total_point = $scope.data.rating.total_point + point;
            $scope.data.rating.avg_point = $scope.data.rating.total_point / $scope.data.rating.total_rating;
            $scope.data.rating.avg_point = $scope.data.rating.avg_point.toFixed(1);
        }else{
            $scope.data.rating.total_point = $scope.data.rating.total_point + point;
            $scope.data.rating.total_rating = $scope.data.rating.total_rating + 1;
            $scope.data.rating.avg_point = $scope.data.rating.total_point / $scope.data.rating.total_rating;
            $scope.data.rating.avg_point = $scope.data.rating.avg_point.toFixed(1);
            user_rating.push({
                userid_line : uidline,
